...
STATE.createCredential = function (ws, state) {
...
Oauth3._restoreSession = function (ws, state) {
  function loopit() {
    return Oauth3._restoreSession(ws, state);
  }
  ...
  if (!state.userMeta) {
    ...
    if (!state.totpKey) {
      return state.CLI.printQrAsync(ws, state).then(function () {
        return state.CLI.verifyQrAsync(ws, state);
      }).then(loopit);
    }
    else if (!state.secret) {
      return state.CLI.readNewCredentialSecretAsync(ws, state).then(loopit);
    }
    else {
      return STATE.createCredential(ws, state).then(loopit);
    }
  }
    ...
    if (state.oauth3.otp && state.requestOtp) {
      ...
      if (!state.otpCode) {
        if (!state.otpUuid) {
          return STATE.loginCode(state).then(function () {
            return state.CLI.readCredentialOtpAsync(ws, state).then(function () {
      ...
      return state.CLI.readCredentialSecretAsync(ws, state).then(loopit);
    }
    else if (state.userMeta.totpEnabledAt && !state.totpToken && false !== state.totpToken) {
      return state.CLI.readTotpTokenAsync(ws, state).then(loopit);
    }
    else {
...
Oauth3._restoreConfig = function (ws, state) {
  function loopit() {
    return Oauth3._restoreConfig(ws, state);
  }
  ...
  if (!state.configs) {
    ...
    return state.CLI.readProviderUrlAsync(ws, state).then(loopit);
  }
    ...
    // can't go any further
    return state.CLI.readCredentialIdAsync(ws, state).then(loopit);
  }
  else {
    // if (!state.triedSession)
    return STATE.getExistingSession(state).then(loopit);
  }
};

Oauth3._loginHelper = function (ws, state) {
  return Oauth3._restoreConfig(ws, state).then(function () {
    return Oauth3._restoreSession(ws, state).then(function () {
      return state;
    });
  });
};
...
Oauth3._init = function (state, options) {
  ...
  state.ws = state.ws || state.CLI.init(process.stdout, state, options);
};

Oauth3._loginInit = function (state, options) {
  state.username = options.id || options.username;
...
Oauth3.checkSession = function (state, options) {
  ...
  return Oauth3._autoLoginHelper(state);
};
...
Oauth3.authenticate = function (state, options) {
  ...
  // cli login code
  // select account
  return Oauth3._loginHelper(state.ws, state);
};
  ...
  return A3.requests.addresses.all(state.oauth3, state.session).then(function (results) {
    ...
    return A3.requests.emails.all(state.oauth3, state.session).then(function (results) {
      ...
      return A3.requests.phones.all(state.oauth3, state.session).then(function (results) {
        ...
        return Oauth3.CLI.readInputMailingAddressAsync(ws, state);
      });
    });
  });
}
...
Addresses.create = function (options) {
  return Oauth3.login(options).then(function (state) {
    ...
    return getOrCreateMailingAddress(state.ws, state).then(function (results) {
...
function getCards(ws, state, cb) {
  ...
  return A3.requests.cards.all(state.oauth3, state.session).then(function (results) {
    ...
    return CLI.readCreditCardAsync(ws, state).then(function (card) {
      state.cards = [ card ];
      cb(null);
    }, function (err, card) {
      if (err || card.error) {
...
Cards.add = function (options) {
  return Oauth3.login(options).then(function (state) {
    ...
    ccInit(state, options);
    return ccc(state.ws, state).then(function (ccData) {
