...
STATE.loginCredential = function (state) {
  return STATE.loginCredentialHelper(state).then(function () {
...
STATE.loginCredentialHelper = function (state) {
...
STATE.getEcho = function (ws, state) {
  state.echo = true;
...
function loginHelper(ws, state, cb) {
  ...
  // TODO load profile by provider / username
    ...
    STATE.getEcho(ws, state).then(loopit);
  }
  else {
...
function getDomain(ws, state, cb) {
  ...
  if (!state.checkoutReady) {
    ...
    STATE.makePurchase(state, loopit);
  }
  else {
    cb(null);
  }
...
function runDomains(ws, state, options) {
...
module.exports.domainsToken = function (options) {
  return module.exports.login(options).then(function (state) {
    ...
    require('../lib/tld-hints').getTldsAsync().then(function (domains) {
      console.log('domains');
      console.log(domains);
      var opts;
      ...
      domains.tlds.some(function (tld) {
        ...
        console.log(options.domain);
        console.log(re, re.test(options.domain));
        if (re.test(options.domain)) {
          opts.tld = tld.tld;
          parts = options.domain.replace(tld, '').split(/\./g);
          opts.sld = parts.pop();
          opts.sub = parts.join('.');
          return true;
        }
...
module.exports.purchaseDomain = function (options) {
  return module.exports.login(options).then(function (state) {
    var err;
    ...
    return PromiseA.promisify(getDomain)(wsInit(process.stdout, state));
  });
};
