...
module.exports.dnsToken = module.exports.domainsToken = function (options) {
  options = options || {};
  var domainname = options.domain || options.name;

  return module.exports.login(options).then(function (state) {
    return getDomainPartsAsync(domainname).then(function (parts) {
      var opts = { sub: parts.sub, sld: parts.sld, tld: parts.tld
                 , device: options.device || state.device.hostname };
      // TODO cache token on disk
      return A3.requests.createDomainToken(state.oauth3, state.session, opts);
    });
  });
};
