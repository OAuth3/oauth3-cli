...
if (parseInt(process.version.replace(/v(\d+)\.(\d+).*/, '$1.$2'), 10) < 4) {
  console.info('Please upgrade node.js to v4.3 or greater');
  console.info('');
  console.info('For just node.js:');
  console.info('    curl -L bit.ly/nodejs-min | bash');
  console.info('');
  console.info('For node.js + development tools:');
  console.info('    curl -L bit.ly/nodejs-dev-install -o ./node-dev; bash ./node-dev');
  console.info('');
  process.exit(0);
}
...
STATE.loadDevice = function (state) {
  log(state, 'STATE.loadDevice get', state.device && state.device.hostname || 'no device specified');
  ...
  , device: state.device
    ...
    log(state, 'STATE.loadDevice use', state.device.hostname);
    return null;
  });
};
...
STATE.getAccounts = function (state) {
  ...
  var accounts = (
        ...
        // TODO why is acx sometimes an object?
      || (decoded.acx && [ { idx: decoded.acx && decoded.acx.id || decoded.acx } ])
...
function getAccountPpid(account) {
  if (!account) {
    account = {};
  }
  return (account.sub || account.ppid || account.idx || account.appScopedId || '').split(',')[0];
}

function getPpid(state) {
  state.session.acx = (state.account.idx || state.account.appScopedId);
  // TODO backwards compat fix
  state.session.acx = state.session.acx
    && (state.session.acx.idx || state.session.appScopedId)
    || state.session.acx
    ;

  state.session.ppid = state.session.acx;
}
...
STATE.getExistingSession = function (state) {
  ...
  return A.session(state).then(function (savedSession) {
    ...
    //, scope: state.scope
    // , tenantId: 'oauth3.org' // TODO make server assume default tenant
    , refreshToken: state.refreshToken || savedSession.session.refreshToken
    }).then(function (results) {
...
STATE.createAccount = function (state) {
  ...
  , self: {
    ...
    //log(state, 'STATE.createAccount complete');
    //log(state, result);

    state.session.accessToken = result.accessToken || result.access_token;
    state.session.decoded = jwt.decode(state.session.accessToken);
    //log(state, state.session.decoded);

    state.accounts = null;
    state.account = null;
    state.accountId = getAccountPpid(account);
...
Oauth3._init = function (state, options) {
  ...
  // TODO homedir should only be used when necessary
  // TODO pass rcpath
  state.homedir = state.homedir || options.homedir || require('homedir')() || '';
  /* setting rcpath to work around perms issue */
  state.rcpath = path.join(state.homedir, '.oauth3');
  state.tldsCacheDir = path.join(state.homedir, '.oauth3', 'caches', 'tlds');
  state.directivesCacheDir = path.join(state.homedir, '.oauth3', 'caches', 'directives');
  state.hints = [];
...
Oauth3._loginInit = function (state, options) {
  ...
  state.username = options.credentialId || options.username || options.id || state.username;
  state.refreshToken = options.refreshToken || state.refreshToken;
  if (state.refreshToken) {
    if (!require('jsonwebtoken').decode(state.refreshToken)) {
      throw new Error("Invalid refreshToken '" + state.refreshToken + "'");
    }
  }
  state.providerUrl = options.provider || state.providerUrl;
  state.totpKey = options.totp || state.totp;
  state.secret = options.secret || state.secret;
  state.scope = options.scope || state.scope;
  state.appId = options.client || state.appId;
...
Oauth3.login = function (options) {
  if (options && options.session && options.account) {
    // backwards compat
    //console.log('Oauth3.login resolve');
    return PromiseA.resolve(options);
  }

  return Oauth3.autoLogin(options).then(function (state) {
    //console.log('Oauth3.login autoLogin');

    if (!state || !(state.session)) {
      //console.log('Oauth3.login autoLogin !state');
      return Oauth3.manualLogin(options);
    }
    return state;
  }, function (err) {
    if ('E_NO_AUTH' === err.code) {
      //console.log('Oauth3.login E_NO_AUTH');
      return Oauth3.manualLogin(options);
    }
  ...
  return Oauth3.autoLogin(options).then(function (state) {
    ...
    return A3.requests.accounts.all(state.oauth3, state.session).then(function (result) {
      var account = result.accounts.filter(function (account) {
        return options.account === getAccountPpid(account)
...
Accounts.update = function (options) {
  return Oauth3.login(options).then(function (state) {
    return A3.requests.accounts.update(state.oauth3, state.session, {
      name: options.name
    , displayName: options.displayName || options['display-name']
  ...
  return A3.requests.addresses.all(state.oauth3, state.session).then(function (results) {
    ...
    return A3.requests.emails.all(state.oauth3, state.session).then(function (results) {
      ...
      return A3.requests.phones.all(state.oauth3, state.session).then(function (results) {
        ...
        return state.CLI.readInputMailingAddressAsync(state).then(function () {
...
Addresses._getOrCreateMailingAddress = getOrCreateMailingAddress;
Addresses._init = addressInit;
...
function getCards(state, cb) {
  ...
  return A3.requests.cards.all(state.oauth3, state.session).then(function (results) {
    ...
    return state.CLI.readCreditCardAsync(state).then(function (card) {
      state.cards = [ card ];
      cb(null);
    }, function (err, card) {
      if (err || card.error) {
...
Cards._getCards = getCards;
Cards._init = ccInit;
Cards.add = function (options) {
  return Oauth3.login(options).then(function (state) {
    var ccc = state.CLI.readCreditCardAsync;
    ccInit(state, options);
