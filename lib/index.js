...
function dollarBillCheck(stream, state) {
  ...
  */
  stream.write("\n");
  stream.write("Please resize this terminal to >= " + minCols + "x" + minRows + "\n");
  stream.write("(you may need to use CTRL- or CMD- if you screen is small)\n");
  stream.write("\n");
  stream.write("Fun Fact:\n");
  stream.write("  Did you know that the default terminal size of 80x24\n");
  stream.write("  was modeled after the size of a dollar bill?\n");  // 2
  stream.write("See http://programmers.stackexchange.com/q/148677\n");                                // 1
  stream.write("\n");
}
...
function qr(ws, state) {
  ...
  state.minRows = state.msgs.length;
