...
CLI.getOtp = function (ws, state, cb) {
...
STATE.domains.purchase = function (state) {
...
FNS.searchDomain = function (ws, state, cb) {
...
function addDomainsToCart(ws, state, cb) {
  ...
  FNS.searchDomain(ws, state, function (err) {
    state.checkoutReady = true;
    cb(err, null);
  });
}
...
function loginHelper(ws, state, cb) {
  ...
  // TODO load profile by provider / username
    ...
    if (state.oauth3.otp && state.requestOtp) {
      ...
      if (!state.otpCode) {
        if (!state.otpUuid) {
          STATE.loginCode(state).then(function () {
            CLI.getOtp(ws, state, function () {
...
Domains._cli.purchase = function (ws, state, cb) {
  ...
  if (!state.checkoutReady) {
    ...
    STATE.domains.purchase(state).then(function () {
      cb(null);
    }, cb);
  }
};
Domains.purchase = function (options) {
  ...
  return Oauth3.login(options).then(function (state) {
    ...
    return initDomainsAsync(state, options).then(function () {
      return PromiseA.promisify(Domains._cli.purchase)(wsInit(process.stdout, state), state).then(function () {
  ...
  return Oauth3.login(options).then(function (state) {
    return PromiseA.all(domainnames.split(',').map(function (domainname) {
      ...
      return getDomainPartsAsync(domainname).then(function (domainparts) {
        ...
        return A3.requests.domains.release(state.oauth3, state.session, options);
      });
    }));
  });
};
...
var Files = Oauth3.Files = {};
Files._sha1part = function (hash, pathname, start, end) {
	var fs = PromiseA.promisifyAll(require('fs'));

  return new PromiseA(function (resolve, reject) {
    var fr;
    fr = fs.createReadStream(pathname, {
      start: start
    , end: end
    });
    fr.pipe(hash, { end: false });
    fr.on('end', function () {
      resolve();
    });
    fr.on('error', reject);
...
Files._sha1sum = function (options) {
  var crypto = require('crypto');
  var hash = crypto.createHash('sha1');
  hash.setEncoding('hex');

  return Files._sha1part(hash, options.pathname, 0, options.stat.size).then(function () {
    hash.end();
    return hash.read();
  });
};
Files._sha1parts = function (hash, pathname, parts, blockSize) {
  function next() {
    var part;
    ...
    console.log(':', part, part + blockSize);
    return Files._sha1part(hash, pathname, part, part + blockSize).then(next);
  }

  return next();
};
Files._sha1qsum = function (options) {
  var crypto = require('crypto');
  var surveySize = (1024 * 16 * 4); // (1024 * 1024); //
  var chunks = 16; // 1024; //
  // several 64b sha1 blocks per survey block;
  var blockSize = surveySize / chunks;
  var gap = Math.floor(options.stat.size / chunks);
  var parts = [];
  var i;
  var hash = crypto.createHash('sha1');
  ...
  if (options.stat.size <= surveySize * 2) {
    return null;
  }
  ...
  return Files._sha1parts(hash, options.pathname, parts, blockSize);
};
Files.upload = function (options) {
	var fs = PromiseA.promisifyAll(require('fs'));

  options.pathname = options.pathname || options.filepath;

  return fs.statAsync(options.pathname).then(function (stat) {
    var promise;

    options.share = '_';
    //options.pathname
    options.stat = stat;
    options.forceSha1 = options.sha1;
    options.sha1 = undefined;
    //options.id = options.id;

    return Files._sha1qsum(options).then(function (sha1qsum) {
      if (!sha1qsum) {
        return Files._sha1sum(options).then(function (sha1sum) {
          options.sha1 = sha1sum;
        });
      }
      options.qhash = sha1qsum;

      console.log('sha2 qsum');
      console.log(sha1qsum);

      if (!options.sha1 && (options.stat.size <= (128 * 1024 * 1024))) {
        promise = Files._sha1sum(options);
      }
      else {
        promise = PromiseA.resolve(options.sha1);
      }

      return promise.then(function (sha1sum) {
        options.sha1 = sha1sum;
        ...
        return Oauth3.login(options).then(function (state) {
          return A3.requests.upload(state.oauth3, state.session, options);
        });
      });
    });
  });
};
Files.shares = function (options) {
  return Oauth3.login(options).then(function (state) {
    return A3.requests.files.shares(state.oauth3, state.session, options);
  });
};
Files.list = function (options) {
  return Oauth3.login(options).then(function (state) {
    return A3.requests.files.list(state.oauth3, state.session, options);
  });
};
