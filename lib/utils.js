...
A3.request = require('./api/request').request;
...
*/
  ...
  return A3.request(A3.resourceOwnerPassword(directive, opts)).then(function (result) {
    ...
    console.error('ERROR utils.js 07');
    return PromiseA.reject(err);
  });
};
A3.requests.loginCode = function (directive, opts) {
  return A3.request(A3.loginCode(directive, opts));
};
...
// extras
//
A3.requests.balance = function (directive, session) {
  var dir = directive.echo || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.payments/accounts/:accountId/balance'
  , bearer: 'Bearer'
  };
  var url = dir.url
...
A3.requests.oldPurchase = function (directive, session, opts) {
  var dir = directive.purchase || {
    method: 'POST'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.payments/accounts/:accountId/wallet'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.loginCode = function (directive, opts) {
  //
  ...
  , "request_otp": opts.requestOtp || undefined
...
// Accounts
A3.requests.accounts = {};
A3.requests.accounts.all = function (directive, opts) {
...
A3.requests.accounts.destroy = function (directive, session, options) {
  var dir = directive.create_account || {
    method: 'DELETE'
  ...
  var url = dir.url
    .replace(/:accountId/, options.accountId)
  ;
...
// Addresses
A3.requests.addresses = {};
A3.requests.addresses.all = function (directive, session) {
  var dir = directive.addresses || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.me/accounts/:accountId/addresses'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.requests.addresses.create = function (directive, session, opts) {
...
// Cards
A3.requests.cards = {};
A3.requests.cards.all = function (directive, session) {
  var dir = directive.cards || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.payments/accounts/:accountId/cards'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.requests.cards.create = function (directive, session, opts) {
...
A3.requests.cards.update = function (directive, session, opts) {
  var dir = directive.createCard || {
    method: 'POST'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.payments/accounts/:accountId/cards/:nick/:brand'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.requests.cards.remove = function (directive, session, query) {
  var dir = directive.cards || {
    method: 'DELETE'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.payments/accounts/:accountId/cards/:last4/:brand'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
// Emails
A3.requests.emails = {};
A3.requests.emails.all = function (directive, session) {
  var dir = directive.emails || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.me/accounts/:accountId/emails'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.requests.emails.create = function (directive, session, opts) {
...
// Files
A3.requests.files = {};
A3.requests.files.shares = function (directive, session/*, params*/) {
  var dir = directive.fileShares || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.files/accounts/:accountId/shares'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
  ...
  console.log('DEBUG requests.files.shares url');
  console.log(url);
...
A3.requests.files.list = function (directive, session, params) {
  var dir = directive.fileList || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.files/accounts/:accountId/shares/:share/files/:path'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
    .replace(/:share/, params.share)
    .replace(/:path/, params.path || '')
  ;
  var query = {
    depth: params.depth
  };
  if ('undefined' !== typeof query.depth) {
...
A3.requests._uploadHelper = function (directive, session, params) {
  var dir = directive.upload || {
    method: 'POST'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.files/accounts/:accountId/files'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.requests.upload = function (directive, session, params) {
  return A3.requests._uploadHelper(directive, session, params).then(function (meta) {

    if ('true' === String(meta.completed) || meta.length === meta.end) {
    ...
    var dir = directive.uploadPart || {
      method: 'POST'
    , url: 'https://' + directive.provider_url + '/api/com.daplie.files/accounts/:accountId/files/:type/:id'
    , bearer: 'Bearer'
    };
    // Torrents are 256k, 512k, or 1mb chunks
    // this means ideally we would have 2,048 parts per 1gb file
    // (each with its own sha1)
    // We could upload perhaps 100 mb per form
    var chunkSize = (10 * 1024 * 1024);
    var numParts = Math.ceil((params.stat.size - meta.end) / chunkSize);
    var parts = [];
    var startAt = meta.end + 1;
    var endAt = 0;
    var len;

    if (meta.sha1) {
      params.idtype = 'sha1';
      params.idval = meta.sha1;
    }
    else if (meta.id) {
      params.idtype = 'id';
      params.idval = meta.id;
    }
    else if (meta.qhash) {
      params.idtype = 'qhash';
      params.idval = meta.qhash;
    }

    // TODO upload two+ in parallel
    while (numParts) {
      endAt = Math.min((startAt + chunkSize) - 1, params.stat.size);
      len = endAt - startAt;
    ...
    function uploadPart() {
      ...
      var url = dir.url
        .replace(/:accountId/, session.acx)
        .replace(/:type/, params.idtype)
        .replace(/:id/, params.idval) // params.sha1 || params.id || params.qhash
      ;
      url += '?' + A3.querystringify({
        start: part.start
      , end: part.end // params.stat.size
      , complete: (part.end === params.stat.size)
      , length: params.stat.size
      });

      // TODO allow buffer instead of filename
      // TODO also allow stream (and unspecified length), but stream 10mb at a time
      console.log('DEBUG start, end:', typeof part.start, part.start, part.end, part.length);
      var formData = {
        attachments: [
          fs.createReadStream(params.pathname, {
            start: part.start
...
// Phones
A3.requests.phones = {};
A3.requests.phones.all = function (directive, session) {
  var dir = directive.phones || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.me/accounts/:accountId/phones'
  , bearer: 'Bearer'
  };
  var url = dir.url
    .replace(/:accountId/, session.acx)
...
A3.requests.phones.create = function (directive, session, opts) {
...
require('./daplie-domains').init(A3);
