...
A3.request.upload = require('./api/request').upload;
...
A3.requests.upload = function (directive, session, params) {
  ...
  var dir = directive.createEmail || {
    method: 'POST'
  , url: 'https://' + directive.provider_url + '/api/com.daplie.files/accounts/:accountId/parts/:hash/:start/:end'
  , bearer: 'Bearer'
  };
  var chunkSize = (100 * 1024);
  var numParts = Math.ceil(params.stat.size / chunkSize);
  var parts = [];
  var startAt = 0;
  var endAt = 0;
  var len;
  ...
  while (numParts) {
    endAt = Math.min(startAt + chunkSize, params.stat.size);
    len = endAt - startAt;
  ...
  function uploadPart() {
    ...
    var url = dir.url
      ...
      .replace(/:start/, part.start)
      .replace(/:end/, part.end)
    ;
