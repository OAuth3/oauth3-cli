...
A3.requests.createCard = function (directive, session, opts) {
...
A3.requests.purchase = function (directive, session, opts) {
...
A3.requests.purchaseDomains = function (directive, session, opts) {
...
A3.requests.createDomainToken = function (directive, session, opts) {
  ...
  , url: 'https://' + directive.provider_url
      + '/api/com.enom.reseller/accounts/:accountId/tokens/:device/:tld/:sld/:sub'
  , bearer: 'Bearer'
  };
  var url = dir.url
...
A3.requests.getDomainRecords = function (directive, session, opts) {
  var dir = directive.domainRecords || {
    method: 'GET'
  , url: 'https://' + directive.provider_url + '/api/com.enom.reseller/accounts/:id/records/:tld/:sld'
  , bearer: 'Bearer'
  };
...
A3.requests.addDeviceToDomain = function (directive, session, opts) {
  var dir = directive.devices || {
    method: 'GET'
  , url: 'https://' + directive.provider_url
      + '/api/com.enom.reseller/accounts/:accountId/records/:device/:tld/:sld/:sub'
  , bearer: 'GET'
  };
  var url = dir.url
    .replace(/:device/, opts.device)
  ...
  [ 'update', 'ttl', 'priority', 'addresses' ].forEach(function (key) {
  ...
  //console.log('DEBUG utils.js addDeviceToDomain query', query);
  //process.exit(1);
  console.log('DEBUG utils.js addDeviceToDomain url', url);
  return A3.request({
    method: dir.method || 'GET'
  , url: url
  }).then(function (results) {
...
A3.requests.deleteDevice = function (directive, session, opts) {
  ...
  console.log('DEBUG utils.js deleteDevice url', url);
  return A3.request({
